...
function hideSkeleton(){
	setTimeout(function(){
		...
		for(i = 0; i < xuiSkeleton.length; i++){
			xuiSkeleton[i].classList.add("hidden");
		}
		for(i = 0; i < xuiSkeletonBtnSmall.length; i++){
...
function modal(){
	document.onclick = function(e){
		let modalOpen = e.target.getAttribute("xui-modal-open");
		let modalClose = e.target.getAttribute("xui-modal-close");
		if(modalOpen !== null){
			document.querySelector('[xui-modal="'+ modalOpen +'"]').style.transform = "scale(1)";
			document.querySelector('body').style.overflow = "hidden";
		}
		if(modalClose !== null){
			document.querySelector('[xui-modal="'+ modalClose +'"]').style.transform = "scale(0)";
...
let navbarMenu = () => {
	var xuiNavbar = document.getElementsByClassName('xui-navbar');
	if (xuiNavbar.length > 0) {
		...
		document.querySelector('.xui-navbar').addEventListener('click', (e) => {
		  if (e.target.closest('.xui-navbar .links .main a')) {
...
let lazyLoadings = () => {
	(function() {
		...
		(function() {
		var elements = document.querySelectorAll('[xui-img-src]');
		var index = 0;
		var lazyLoad = function() {
			if(index >= elements.length) return;
			var item = elements[index];
			...
			let typeWriter = (text, i, cb) => {
				if(textArray.length > 1){
					...
					if ( i < text.length+1 ) {
						...
						document.querySelector(".xui-effect-typewriter[xui-effect-typewriter=\""+ target +"\"]").classList.add("xui-effect-typewriter-ready");
						document.querySelector(".xui-effect-typewriter[xui-effect-typewriter=\""+ target +"\"] span.xui-effect-typewriter-cursor").style.display = "none";
					}
				}
			};
...
let scrollOnAnimation = () => {
	...
	let xuiAOSCheck = document.querySelector("body").classList.contains('xui-aos');
	if(xuiAOSCheck){
...
window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', event => {
	...
	if(newColorScheme == "dark"){
		document.querySelector("[xui-mode=\"auto\"]").classList.add("xui-dark-mode");
	}
	else {
		document.querySelector("[xui-mode=\"auto\"]").classList.remove("xui-dark-mode");
	}
});
